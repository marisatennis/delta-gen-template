...
{
    { "Stock Value", new List<string>() { "Today", "Yesterday", "Today vs Yesterday", "MTD", "PMTD", "MTD vs PMTD", "YTD", "PYTD", "YTD vs PYTD" } },
    { "Stock Volume", new List<string>() { "Today", "Yesterday", "Today vs Yesterday", "MTD", "PMTD", "MTD vs PMTD", "YTD", "PYTD", "YTD vs PYTD" } },
};
...
foreach(var kvp in measureDerivatives)
{
    ...
    foreach (var derivativeName in derivatives)
    {
        ...
        if (derivativeExpressions.ContainsKey(derivativeName))
        {
            ...
            var newMeasure = Model.Tables["f_inventorysnapshot"].AddMeasure(
            ...
            if (newMeasure.Name.Contains("vs"))
            {
                ...
                var newArrowMeasure = Model.Tables["f_inventorysnapshot"].AddMeasure(
...
List<string> measuresWithComparators = new List<string>() { "Stock Value", "Stock Volume" };
    ...
    if (measuresWithComparators.Contains(mainMeasureName))
    {
        foreach (var comparatorName in ComparatorExpressions.Keys)
        {
            if (ComparatorExpressions.ContainsKey(comparatorName))
            {
                ...
                var newComparatorMeasure = Model.Tables["f_inventorysnapshot"].AddMeasure(
