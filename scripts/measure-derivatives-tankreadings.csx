...
{
    { "Total Tank Variance", new List<string>() { "Today", "Yesterday", "Today vs Yesterday" } },
    { "Total Tank Capacity %", new List<string>() { "Today", "Yesterday", "Today vs Yesterday" } },
    { "No. Variances", new List<string>() { "Today", "Yesterday", "Today vs Yesterday" } },
    { "Total Volume Held", new List<string>() { "Today", "Yesterday", "Today vs Yesterday" } },
};
...
foreach(var kvp in measureDerivatives)
{
    ...
    foreach (var derivativeName in derivatives)
    {
        ...
        if (derivativeExpressions.ContainsKey(derivativeName))
        {
            ...
            var newMeasure = Model.Tables["f_tankreadings"].AddMeasure(
            ...
            if (newMeasure.Name.Contains("vs"))
            {
                ...
                var newArrowMeasure = Model.Tables["f_tankreadings"].AddMeasure(
...
foreach (var kvp in measureDerivatives)
{
    ...
    foreach (var comparatorName in ComparatorExpressions.Keys)
    {
        if (ComparatorExpressions.ContainsKey(comparatorName))
        {
            ...
            var newComparatorMeasure = Model.Tables["f_tankreadings"].AddMeasure(
