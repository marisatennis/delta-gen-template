...
{
    { "Total Spend", new List<string>() { "Today", "Yesterday", "Today vs Yesterday", "MTD", "PMTD", "MTD vs PMTD", "YTD", "PYTD", "YTD vs PYTD" } },
    { "No. Purchase Orders", new List<string>() { "Today", "Yesterday", "Today vs Yesterday", "MTD", "PMTD", "MTD vs PMTD", "YTD", "PYTD", "YTD vs PYTD" } },
    { "Spend Volume", new List<string>() { "YTD", "PYTD" } }
};
...
foreach(var kvp in measureDerivatives)
{
    ...
    foreach (var derivativeName in derivatives)
    {
        ...
        if (derivativeExpressions.ContainsKey(derivativeName))
        {
            ...
            var newMeasure = Model.Tables["f_purchaseorders"].AddMeasure(
            ...
            if (newMeasure.Name.Contains("vs"))
            {
                ...
                var newArrowMeasure = Model.Tables["f_purchaseorders"].AddMeasure(
...
List<string> measuresWithComparators = new List<string>() { "Total Spend", "No. Purchase Orders" };
    ...
    if (measuresWithComparators.Contains(mainMeasureName))
    {
        foreach (var comparatorName in ComparatorExpressions.Keys)
        {
            if (ComparatorExpressions.ContainsKey(comparatorName))
            {
                ...
                var newComparatorMeasure = Model.Tables["f_purchaseorders"].AddMeasure(
